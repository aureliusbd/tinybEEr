...
tinybEEr_TCS34725::tinybEEr_TCS34725(TwoWire *pWire , uint8_t I2C_addr,eIntegrationTime_t it, eGain_t gain)
{
...
boolean tinybEEr_TCS34725::begin(void)
{
  Wire.begin();
  uint8_t x =0;
...
void tinybEEr_TCS34725::enable(void)
{
...
void tinybEEr_TCS34725::disable(void)
{
  uint8_t reg = 0;
...
void tinybEEr_TCS34725::setIntegrationtime(eIntegrationTime_t it)
{
  uint8_t data = it;
...
void tinybEEr_TCS34725::setGain(eGain_t gain)
{
  uint8_t data = gain;
...
void tinybEEr_TCS34725::getRGBC (uint16_t *r, uint16_t *g, uint16_t *b, uint16_t *c)
{
...
uint16_t tinybEEr_TCS34725::calculateColortemperature(uint16_t r, uint16_t g, uint16_t b)
{
...
uint16_t tinybEEr_TCS34725::calculateLux(uint16_t r, uint16_t g, uint16_t b)
{
  float illuminance;
...
void tinybEEr_TCS34725::lock(void)
{
	uint8_t r;
...
void tinybEEr_TCS34725::unlock(void)
{
	uint8_t r;
  readReg(TCS34725_ENABLE, &r,1);
  r &= ~TCS34725_ENABLE_AIEN;
...
void tinybEEr_TCS34725::clear(void) {
...
void tinybEEr_TCS34725::setIntLimits(uint16_t low, uint16_t high) {
...
void tinybEEr_TCS34725::setGenerateinterrupts(void)
{
  uint8_t data = TCS34725_PERS_NONE;
  writeReg(TCS34725_PERS, &data,1);
}

void tinybEEr_TCS34725::writeReg(uint8_t Reg, void *pData, uint8_t len)
{
...
int16_t tinybEEr_TCS34725::readReg(uint8_t Reg, uint8_t *Data, uint8_t len)
{
  int i = 0;
  _pWire->beginTransmission(this->_I2C_addr);
  _pWire->write(TCS34725_COMMAND_BIT | Reg);
...
uint16_t tinybEEr_TCS34725::readRegword(uint8_t reg)
{
  uint8_t data_buf[2];
